...
import { debounceTime, mergeAll, switchAll, toArray } from 'rxjs/operators';
...
export class AppComponent implements OnInit, OnDestroy {
  ...
  changeCharactersInput(element): void {
    ...
    this.searchTermByCharacters.next(element.target.value);
    // YOUR CODE ENDS HERE
  }
  ...
  initCharacterEvents(): void {
    ...
    this.charactersResults$ = this.searchTermByCharacters.pipe(
      // YOUR CODE STARTS HERE
      filter((charactersResults) => charactersResults.length > 2),
      debounceTime(1000),
      switchMap((charactersResults) =>
        this.mockDataService.getCharacters(charactersResults)
      )
      // YOUR CODE ENDS HERE
    );
  }
